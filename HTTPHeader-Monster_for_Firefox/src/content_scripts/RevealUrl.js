...
function setup_dialog_window(node, new_url, linkUrl) {
	...
	try {
		...
		var fullURLToCSS = browser.runtime.getURL("popup/message-box.css");
		
		var td_link = document.createElement('link');
		td_link.setAttribute("rel", 'stylesheet');
		td_link.setAttribute("type", 'text/css');
		td_link.setAttribute("href", fullURLToCSS);
		cont1.appendChild(td_link);
		...
		if ((it2.style.top == '' || it2.style.top == 0 || it2.style.top == "0px") && (it2.style.left == '' || it2.style.left == 0 || it2.style.left == "0px")) {
			...
			//console.debug("Cursor_clientX: " + Cursor_clientX);
			//console.debug("Cursor_clientY: " + Cursor_clientY);
			//console.debug("Cursor_layerX: " + Cursor_layerX);
			//console.debug("Cursor_layerY: " + Cursor_layerY);
			...
			// X -position
			//it2.style.left = x + 'px';
			it2.style.left = Cursor_clientX + 'px';
